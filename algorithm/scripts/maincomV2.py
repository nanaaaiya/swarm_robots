...
# --------------------------
INSTANCE_SEED = 540391549   # <-- seed used to generate the fixed instance (changeable)
...
# robot_positions = rng_inst.rand(n_robots, 2) * 5
...
dist_task_to_drop = np.linalg.norm(task_positions - dropoff_positions, axis=1)  # len m_tasks
...
def run_milp_assignment_and_tsp():
    ...
    prob += pl.lpSum((dist_robot_to_task[i,j] + dist_task_to_drop[j]) * x[i][j]
                     for i in range(n_robots) for j in range(m_tasks))
    ...
    for i in range(n_robots):
        ...
        while remaining:
            # pick nearest pickup
            nearest = min(remaining, key=lambda t: np.linalg.norm(cur_pos - task_positions[t]))
...
def run_pso(run_seed, num_particles=30, max_iter=100, w=0.7, c1=1.5, c2=1.5):
    ...
    class Particle:
        ...
        def evaluate(self):
            ...
            for i in range(n_robots):
                ...
                for t in assigned_tasks:
                    pick = tuple(task_positions[t]); drop = tuple(dropoff_positions[t])
        ...
        def mutate(self):
            ...
            self.task_order[a], self.task_order[b] = self.task_order[b], self.task_order[a]
            if rng.rand() < 0.05:
    ...
    total_distances = {}
    for i in range(n_robots):
        assigned_tasks = [t for t in global_best_task_order if global_best_assignment[t] == i]
        route = []
        prev = tuple(robot_positions[i])
        dist_sum = 0.0
        for t in assigned_tasks:
...
# ACO (uses run_seed)
# --------------------------
def run_aco(run_seed, num_ants=30, num_iterations=100, alpha=1.0, beta=2.0, rho=0.1, q=100.0):
    rng = np.random.RandomState(int(run_seed))
